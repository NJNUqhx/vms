<template>
  <div>
    <el-input v-model="name" placeholder="请输入仓库名" style="margin-bottom: 5px; width: 200px;"
    ...
    <el-table :data="tableData" :header-cell-style="{background: '#f2f5fc', color: '#555555'} " border>
      ...
      <el-table-column label="仓库名" prop="name" width="180">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    remove(id) {
      this.$axios.get(this.$httpUrl + '/storage/remove?id='+id).then(res => res.data).then(res => {
        console.log(res);
        if (res.code === 200) {
          this.$message({
            message: "操作成功",
    ...
    loadPost() {
      this.$axios.post(this.$httpUrl + '/storage/query', {
